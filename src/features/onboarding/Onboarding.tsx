...
export default function Onboarding() {
  ...
  function start() {
    neuesSpiel({ name: name.trim() || 'Investor:in', nettoEinkommen: netto, fixkosten }, startkapital)
  }

  const frei = netto - fixkosten
        ...
        {step === 1 && (
          <Card className="p-6 animate-fade-in">
            ...
            <div className="mt-5 rounded-xl bg-slate-50 p-4 text-sm">
              <div className="flex items-center justify-between">
                <span className="text-ink-500">Frei verfügbar / Monat</span>
                <span className={`font-bold tabular-nums ${frei < 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
                  {euro(frei)}
                </span>
              </div>
            </div>
            ...
            <div className="mt-5 flex justify-between">
              ...
              <Button onClick={start}>Los geht's 🏠</Button>
            </div>
          </Card>
        )}
      </div>
